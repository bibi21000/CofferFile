...
def test_bz2_crypt(random_path, random_name, buff_size, file_size):
    ...
    datau = b''
    while True:
        size_meta = data[beg:beg + cofferfile.META_SIZE]
        size_struct = datar[beg:beg + cofferfile.META_CHUNK]
        if len(size_struct) == 0:
            break
        size_data = struct.unpack('<I', size_struct)[0]
        chunk = datar[beg + cofferfile.META_SIZE:beg + size_data + cofferfile.META_SIZE]
        beg += size_data + cofferfile.META_SIZE
